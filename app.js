...
function medicationTimingFlag(haystack) {
  return /(до еды|перед едой|после еды|во время еды|с едой|натощак|за\s*(?:15|30)(?:\s*минут)?|утр(?:ом|о|енний)?|вечер(?:ом|ний|нее)?|на ночь|перед сном|рабепразол|эзомепразол|омепразол|ганатон|итоприд|де-нол|де нол|висмут|левотироксин)/.test(haystack);
}
